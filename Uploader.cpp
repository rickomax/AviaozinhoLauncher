...
static bool LooksLikePE(const fs::path& p)
{
    // Minimal detection for PE binaries (EXE/DLL/OCX/SYS/etc.)
    // by checking the DOS header magic "MZ".
    std::ifstream f(p, std::ios::binary);
    if (!f) return false;
    unsigned char hdr[2] = { 0,0 };
    f.read(reinterpret_cast<char*>(hdr), 2);
    return f.gcount() == 2 && hdr[0] == 'M' && hdr[1] == 'Z';
}
    ...
    std::vector<fs::path>& outOffenders)
{
    ...
    for (fs::recursive_directory_iterator it(root, opts, ec), end; !ec && it != end; it.increment(ec))
    {
        ...
        if (IsForbiddenExt(extLower)) {
            ...
            // Also block any file that *looks* like a PE, even if
            // the extension is missing or disguised.
            if (LooksLikePE(p)) bad = true;
        }
